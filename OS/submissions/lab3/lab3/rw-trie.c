//Danila Chenchik (chenchik) and Greer Glenn (greer112) 
// We agree to the UNC Honor Code. We did not give nor receive unauthorized help on this assignment
/*RW TRIE -- allows multiple readers, excludes multiple writers*/
...
static  int closing_delete_thread = 0;
...
void init(int numthreads) {
  ...
  //make the coarse grained lock
  int res1 = pthread_mutex_init(&mutex, NULL);
  //make the condition
  int res2 = pthread_cond_init(&condition, NULL);

  int res3 = pthread_rwlock_init(&rwlock, NULL);
...
int drop_one_node  () {
  if(root){
    ...
    int returnVal = (NULL != _delete(root, fullKey, totalStrLen));    
    return returnVal; 
  }
  return 0;
}
...
void check_max_nodes  () {
  //locking regular mutex
  pthread_mutex_lock(&mutex);

      //wait on the thread?
      if(separate_delete_thread && !closing_delete_thread){
        while(node_count <= max_count){
          pthread_cond_wait(&condition, &mutex);
        }
      }

  //now able to lock rwlock
  pthread_rwlock_wrlock(&rwlock);
